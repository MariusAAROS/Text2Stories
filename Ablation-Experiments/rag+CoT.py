...
save_path = os.path.join(save_dir, "rag+CoT.json")
...
tooledllm = llm.bind_tools([retriever])
...
def elicitation(state: State) -> State:
    keywords = llm.invoke(
        [
            SystemMessage(prompts["RAG"]["retrieval"]["sys"]),
            HumanMessage(prompts["RAG"]["retrieval"]["user"].format(
    ...
    query = tooledllm.invoke(
        [
            SystemMessage(prompts["RAG"]["augmentation"]["sys"]),
            HumanMessage(prompts["RAG"]["augmentation"]["user"].format(
                product=state["product"],
                topic=state["topic"],
    ...
    retrieved_documents = retriever.invoke(input=query.tool_calls[0]["args"])

    try:
        output = llm4needs.invoke(
            [
                SystemMessage(prompts["RAG"]["elicitation_alt"]["sys"].format(
                    product=state["product"],
                    topic=state["topic"]
                )),
                HumanMessage(prompts["RAG"]["elicitation_alt"]["user"].format(
                    product=state["product"],
                    retrieved_documents= retrieved_documents if retrieved_documents else "No documents found.",
                    needs="\n".join([need.need for need in state["needs"]]) if state["needs"] else "No needs yet.",
                ))
            ]
        )
